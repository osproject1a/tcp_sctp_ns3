...
NS_LOG_COMPONENT_DEFINE ("SctpRxBuffer");

namespace ns3 {

TypeId
SctpRxBuffer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SctpRxBuffer")
    .SetParent<Object> ()
    .AddConstructor<SctpRxBuffer> ()
    .AddTraceSource ("NextRxSequence",
                     "Next sequence number expected (RCV.NXT)",
                     MakeTraceSourceAccessor (&SctpRxBuffer::m_nextRxSeq))
  ;
  return tid;
}

/* A user is supposed to create a SctpSocket through a factory. In SctpSocket,
 ...
 * RcvBufSize is passed to SctpRxBuffer by SctpSocketBase::SetRcvBufSize() and in
 * turn, SctpRxBuffer:SetMaxBufferSize(). Therefore, the m_maxBuffer value
 * initialized below is insignificant.
 */
SctpRxBuffer::SctpRxBuffer (uint32_t n)
  : m_nextRxSeq (n), m_gotFin (false), m_size (0), m_maxBuffer (32768), m_availBytes (0)
{
}

SctpRxBuffer::~SctpRxBuffer ()
{
}

SequenceNumber32
SctpRxBuffer::NextRxSequence (void) const
{
  return m_nextRxSeq;
}

void
SctpRxBuffer::SetNextRxSequence (const SequenceNumber32& s)
{
  m_nextRxSeq = s;
}

uint32_t
SctpRxBuffer::MaxBufferSize (void) const
{
  return m_maxBuffer;
}

void
SctpRxBuffer::SetMaxBufferSize (uint32_t s)
{
  m_maxBuffer = s;
}

uint32_t
SctpRxBuffer::Size (void) const
{
  return m_size;
}

uint32_t
SctpRxBuffer::Available () const
{
  return m_availBytes;
}

void
SctpRxBuffer::IncNextRxSequence ()
{
...
// Return the lowest sequence number that this SctpRxBuffer cannot accept
SequenceNumber32
SctpRxBuffer::MaxRxSequence (void) const
{
  if (m_gotFin)
...
void
SctpRxBuffer::SetFinSequence (const SequenceNumber32& s)
{
  NS_LOG_FUNCTION (this);
...
bool
SctpRxBuffer::Finished (void)
{
  return (m_gotFin && m_finSeq < m_nextRxSeq);
}

bool
SctpRxBuffer::Add (Ptr<Packet> p, SctpHeader const& sctph)
{
  NS_LOG_FUNCTION (this << p << sctph);

  uint32_t pktSize = p->GetSize ();
  SequenceNumber32 headSeq = sctph.GetSequenceNumber ();
  ...
  if (headSeq >= tailSeq)
    ...
    {
      uint32_t start = headSeq - sctph.GetSequenceNumber ();
...
Ptr<Packet>
SctpRxBuffer::Extract (uint32_t maxSize)
{
  NS_LOG_FUNCTION (this << maxSize);

  uint32_t extractSize = std::min (maxSize, m_availBytes);
  NS_LOG_LOGIC ("Requested to extract " << extractSize << " bytes from SctpRxBuffer of size=" << m_size);
