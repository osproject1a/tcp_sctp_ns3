 ...
 */
#ifndef SCTP_SOCKET_BASE_H
#define SCTP_SOCKET_BASE_H

#include <stdint.h>
#include <queue>
#include "ns3/callback.h"
#include "ns3/traced-value.h"
#include "ns3/sctp-socket.h"
#include "ns3/ptr.h"
...
#include "sctp-tx-buffer.h"
#include "sctp-rx-buffer.h"
 ...
 * \ingroup sctp
 *
 * \brief A base class for implementation of a stream socket using SCTP.
 *
 * This class contains the essential components of SCTP, as well as a sockets
 * interface for upper layers to call. This serves as a base for other SCTP
 ...
 * this class is modified from the original NS-3 SCTP socket implementation
 * (SctpSocketImpl) by Raj Bhattacharjea <raj.b@gatech.edu> of Georgia Tech.
 */
class SctpSocketBase : public SctpSocket
{
public:
  ...
  /**
   * Clone a SCTP socket, for use upon receiving a connection request in LISTEN state
   */
  SctpSocketBase (const SctpSocketBase& sock);
  virtual ~SctpSocketBase (void);

  // Set associated Node, SctpL4Protocol, Sctp_RttEstimator to this socket
  virtual void SetNode (Ptr<Node> node);
  virtual void SetSctp (Ptr<SctpL4Protocol> sctp);
  virtual void SetRtt (Ptr<Sctp_RttEstimator> rtt);

  // Necessary implementations of null functions from ns3::Socket
  virtual enum SocketErrno GetErrno (void) const;    // returns m_errno
  virtual enum SocketType GetSocketType (void) const; // returns socket type
  virtual Ptr<Node> GetNode (void) const;            // returns m_node
  virtual int Bind (void);    // Bind a socket by setting up endpoint in SctpL4Protocol
  virtual int Bind6 (void);    // Bind a socket by setting up endpoint in SctpL4Protocol
...
protected:
  ...
  virtual void     SetSctpNoDelay (bool noDelay);
  virtual bool     GetSctpNoDelay (void) const;
  ...
  void CompleteFork (Ptr<Packet>, const SctpHeader&, const Address& fromAddress, const Address& toAdress);
  ...
  void PeerClose (Ptr<Packet>, const SctpHeader&); // Received a FIN from peer, notify rx buffer
  ...
  void ProcessEstablished (Ptr<Packet>, const SctpHeader&); // Received a packet upon ESTABLISHED state
  void ProcessListen (Ptr<Packet>, const SctpHeader&, const Address&, const Address&); // Process the newly received ACK
  void ProcessSynSent (Ptr<Packet>, const SctpHeader&); // Received a packet upon SYN_SENT
  void ProcessSynRcvd (Ptr<Packet>, const SctpHeader&, const Address&, const Address&); // Received a packet upon SYN_RCVD
  void ProcessWait (Ptr<Packet>, const SctpHeader&); // Received a packet upon CLOSE_WAIT, FIN_WAIT_1, FIN_WAIT_2
  void ProcessClosing (Ptr<Packet>, const SctpHeader&); // Received a packet upon CLOSING
  void ProcessLastAck (Ptr<Packet>, const SctpHeader&); // Received a packet upon LAST_ACK
  ...
  // Manage data tx/rx
  virtual Ptr<SctpSocketBase> Fork (void) = 0; // Call CopyObject<> to clone me
  virtual void ReceivedAck (Ptr<Packet>, const SctpHeader&); // Received an ACK packet
  virtual void ReceivedData (Ptr<Packet>, const SctpHeader&); // Recv of a data, put into buffer, call L7 to get it if necessary
  virtual void EstimateRtt (const SctpHeader&); // RTT accounting
  virtual void NewAck (SequenceNumber32 const& seq); // Update buffers w.r.t. ACK
  virtual void DupAck (const SctpHeader& t, uint32_t count) = 0; // Received dupack
  ...
  virtual void ReadOptions (const SctpHeader&); // Read option from incoming packets
  virtual void AddOptions (SctpHeader&); // Add option to outgoing packets
...
protected:
  ...
  // Connections to other layers of SCTP/IP
  ...
  Ptr<Sctp_RttEstimator> m_rtt;

  // Rx and Tx buffer management
  TracedValue<SequenceNumber32> m_nextTxSequence; //< Next seqnum to be sent (SND.NXT), ReTx pushes it back
  TracedValue<SequenceNumber32> m_highTxMark;     //< Highest seqno ever sent, regardless of ReTx
  SctpRxBuffer                   m_rxBuffer;       //< Rx buffer (reordering buffer)
  SctpTxBuffer                   m_txBuffer;       //< Tx buffer

  // State-related attributes
  TracedValue<SctpStates_t> m_state;         //< SCTP state
