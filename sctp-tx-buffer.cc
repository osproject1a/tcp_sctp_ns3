...
NS_LOG_COMPONENT_DEFINE ("SctpTxBuffer");

namespace ns3 {

TypeId
SctpTxBuffer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SctpTxBuffer")
    .SetParent<Object> ()
    .AddConstructor<SctpTxBuffer> ()
    .AddTraceSource ("UnackSequence",
                     "First unacknowledged sequence number (SND.UNA)",
                     MakeTraceSourceAccessor (&SctpTxBuffer::m_firstByteSeq))
  ;
  return tid;
}
 ...
 * SndBufSize is passed to SctpTxBuffer by SctpSocketBase::SetSndBufSize() and in
 * turn, SctpTxBuffer:SetMaxBufferSize(). Therefore, the m_maxBuffer value
 * initialized below is insignificant.
 */
SctpTxBuffer::SctpTxBuffer (uint32_t n)
  : m_firstByteSeq (n), m_size (0), m_maxBuffer (32768), m_data (0)
{
}

SctpTxBuffer::~SctpTxBuffer (void)
{
}

SequenceNumber32
SctpTxBuffer::HeadSequence (void) const
{
  return m_firstByteSeq;
}

SequenceNumber32
SctpTxBuffer::TailSequence (void) const
{
  return m_firstByteSeq + SequenceNumber32 (m_size);
}

uint32_t
SctpTxBuffer::Size (void) const
{
  return m_size;
}

uint32_t
SctpTxBuffer::MaxBufferSize (void) const
{
  return m_maxBuffer;
}

void
SctpTxBuffer::SetMaxBufferSize (uint32_t n)
{
  m_maxBuffer = n;
}

uint32_t
SctpTxBuffer::Available (void) const
{
  return m_maxBuffer - m_size;
}

bool
SctpTxBuffer::Add (Ptr<Packet> p)
{
...
uint32_t
SctpTxBuffer::SizeFromSequence (const SequenceNumber32& seq) const
{
...
Ptr<Packet>
SctpTxBuffer::CopyFromSequence (uint32_t numBytes, const SequenceNumber32& seq)
{
...
void
SctpTxBuffer::SetHeadSequence (const SequenceNumber32& seq)
{
  NS_LOG_FUNCTION (this << seq);
  m_firstByteSeq = seq;
}
...
void
SctpTxBuffer::DiscardUpTo (const SequenceNumber32& seq)
{
  NS_LOG_FUNCTION (this << seq);
