 ...
 * 
 * This class allocates "endpoint" objects (ns3::Ipv4EndPoint) for SCTP,
...
public:
  ...
  /**
   * \return A smart Socket pointer to a SctpSocket allocated by this instance
...
private:
  friend class SctpSocketBase;
  void SendPacket (Ptr<Packet>, const SctpHeader &,
                   Ipv4Address, Ipv4Address, Ptr<NetDevice> oif = 0);
  void SendPacket (Ptr<Packet>, const SctpHeader &,
                   Ipv6Address, Ipv6Address, Ptr<NetDevice> oif = 0);
  SctpL4Protocol (const SctpL4Protocol &o);
  SctpL4Protocol &operator = (const SctpL4Protocol &o);

  std::vector<Ptr<SctpSocketBase> > m_sockets;
