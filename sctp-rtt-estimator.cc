...
NS_LOG_COMPONENT_DEFINE ("Sctp_RttEstimator");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (Sctp_RttEstimator);

TypeId 
Sctp_RttEstimator::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Sctp_RttEstimator")
    .SetParent<Object> ()
    .AddAttribute ("MaxMultiplier", 
                   "Maximum RTO Multiplier",
                   UintegerValue (64),
                   MakeUintegerAccessor (&Sctp_RttEstimator::m_maxMultiplier),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("InitialEstimation", 
                   "Initial RTT estimation",
                   TimeValue (Seconds (1.0)),
                   MakeTimeAccessor (&Sctp_RttEstimator::m_initialEstimatedRtt),
                   MakeTimeChecker ())
    .AddAttribute ("MinRTO", 
                   ...
                   MakeTimeAccessor (&Sctp_RttEstimator::SetMinRto,
                                     &Sctp_RttEstimator::GetMinRto),
                   MakeTimeChecker ())
  ;
  return tid;
}
...
void 
Sctp_RttEstimator::SetMinRto (Time minRto)
{
  NS_LOG_FUNCTION (this << minRto);
  m_minRto = minRto;
}
Time 
Sctp_RttEstimator::GetMinRto (void) const
{
  return m_minRto;
}
void 
Sctp_RttEstimator::SetCurrentEstimate (Time estimate)
{
  NS_LOG_FUNCTION (this << estimate);
  m_currentEstimatedRtt = estimate;
}
Time 
Sctp_RttEstimator::GetCurrentEstimate (void) const
{
...
Sctp_RttEstimator::Sctp_RttEstimator ()
  : m_next (1), m_history (),
    m_nSamples (0),
    m_multiplier (1)
{ 
...
Sctp_RttEstimator::Sctp_RttEstimator (const Sctp_RttEstimator& c)
...
void Sctp_RttEstimator::SentSeq (SequenceNumber32 seq, uint32_t size)
{ 
...
Time Sctp_RttEstimator::AckSeq (SequenceNumber32 ackSeq)
{ 
...
void Sctp_RttEstimator::ClearSent ()
{ 
  NS_LOG_FUNCTION (this);
...
void Sctp_RttEstimator::IncreaseMultiplier ()
{
  NS_LOG_FUNCTION (this);
...
void Sctp_RttEstimator::ResetMultiplier ()
{
  NS_LOG_FUNCTION (this);
  m_multiplier = 1;
}

void Sctp_RttEstimator::Reset ()
{ 
  NS_LOG_FUNCTION (this);
...
RttMeanDeviation::RttMeanDeviation (const RttMeanDeviation& c)
  : Sctp_RttEstimator (c), m_gain (c.m_gain), m_variance (c.m_variance)
{
  NS_LOG_FUNCTION (this);
}
...
Ptr<Sctp_RttEstimator> RttMeanDeviation::Copy () const
{
  NS_LOG_FUNCTION (this);
